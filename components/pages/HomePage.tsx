...
/**
 * 首頁組件。
 * 負責顯示應用程式的主要入口畫面，包括問候語、標題、描述和下載履歷 (CV) 的功能。
 * 還包含一個動態的下雨背景效果。
 */
const HomePage: React.FC = () => {
  ...
  // Ref：用於引用將要被轉換為 PDF 的隱藏內容元素
  const pdfContentRef = useRef<HTMLDivElement>(null);
  // 狀態：觸發 PDF 內容渲染的語言。當點擊下載時，設置此狀態以確保 PDF 以當前語言渲染。
  const [renderPdfTriggerLang, setRenderPdfTriggerLang] = useState<string | null>(null);
  // 狀態：當前應用程式的主題（淺色或深色）
  const [currentTheme, setCurrentTheme] = useState<Theme>('dark');

  // Effect：監聽 <body> 的 class 變化以同步主題狀態。
  // 這使得此組件能響應在 App.tsx 中進行的主題切換。
  useEffect(() => {
   ...
   * 處理下載履歷按鈕的點擊事件。
   * 這是一個複雜的異步函數，它會：
   * 1. 觸發一個隱藏的 React 組件 (CVDocument) 以當前語言重新渲染。
   * 2. 使用 html2canvas 將該組件渲染成一個大的 Canvas 圖像。
   * 3. 使用 jsPDF 將該圖像切割成多頁並生成 PDF 文件。
   */
  const handleDownloadCV = async () => {
    setIsGeneratingPdf(true);
    setRenderPdfTriggerLang(i18n.language); // 設置語言以觸發 PDF 內容的渲染

    // 使用兩層 requestAnimationFrame 以確保 React 完成渲染並且瀏覽器完成佈局和繪製。
    // 這是捕獲最新 DOM 狀態的可靠方法。
    requestAnimationFrame(() => {
      requestAnimationFrame(async () => {
        if (pdfContentRef.current) {
          try {
            // 關鍵步驟：等待網頁字體完全加載。
            // 這可以防止 html2canvas 在字體還未準備好時就截圖，從而導致 PDF 中的文字使用後備字體。
            await document.fonts.ready;

            const canvas = await html2canvas(pdfContentRef.current, {
              scale: 2, // 提高分辨率以獲得更清晰的 PDF
              useCORS: false, 
              backgroundColor: currentTheme === 'dark' ? '#18181B' : '#ffffff', // 根據主題設置背景色
              width: 780, // 強制畫布寬度以匹配設計
              windowWidth: 780, 
            ...
            if (canvas.width === 0 || canvas.height === 0) {
                throw new Error("html2canvas 產生的畫布尺寸為零。");
            }

            const imgData = canvas.toDataURL('image/png');
            if (!imgData || imgData === 'data:,') {
                throw new Error("html2canvas 產生的圖像數據為空。");
            }
            ...
            while(currentSourcePxY < totalSourcePxHeight) {
              ...
              const sourceRectHeightPx = Math.min(
                  (pdfHeightMm / PxToMmScale), 
                  totalSourcePxHeight - currentSourcePxY 
              );

              if (sourceRectHeightPx <= 0) {
                break;
              }
              ...
              if (!tempCtx) {
                throw new Error("無法從臨時畫布獲取 2D 上下文以進行頁面切割。");
              }
              ...
              if (!pageImgData || pageImgData === 'data:,') {
                throw new Error(`為第 ${pageCount} 頁生成的圖像數據為空。`);
              }
            ...
            const themePart = currentTheme === 'dark'
              ? t('sidebar.darkMode', { lng: i18n.language })
              : t('sidebar.lightMode', { lng: i18n.language });
            
            const themeStr = `${themePart}${i18n.language === 'zh-Hant' ? '版' : ''}`;
            
            const docTypePart = i18n.language === 'zh-Hant'
              ? '履歷表(中文)'
              : 'Resume(English)';
            ...
            setRenderPdfTriggerLang(null); // 清理觸發器狀態
          }
        } else {
          setIsGeneratingPdf(false);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center text-center relative overflow-hidden -m-6 md:-m-12">
      ...
      <div 
        ...
        style={{ backgroundImage: "url('/images/home-bg.jpg')", zIndex: 0 }}
      ></div>
      {/* 主題顏色遮罩層 */}
      <div className={`absolute inset-0 bg-theme-secondary`} style={{ zIndex: 0, opacity: currentTheme === 'light' ? 0.08 : 0.7 }}></div>
      ...
      <motion.div 
        className="relative z-10 p-6 pt-16 sm:pt-20 md:pt-6"
        variants={staggerContainerVariants(0.3)}
        initial="initial"
        animate="animate"
      >
        <motion.h1 
          className="text-4xl sm:text-5xl md:text-7xl font-bold text-theme-primary"
        ...
        <motion.p 
          className="mt-6 max-w-2xl mx-auto text-theme-secondary text-lg"
          variants={fadeInUpItemVariants}
        >
        ...
        >
          <button 
            ...
            className={`button-theme-accent font-semibold py-3 px-8 rounded-full text-lg transition-all duration-300 shadow-lg flex items-center mx-auto disabled:opacity-70 disabled:cursor-not-allowed`}
          >
