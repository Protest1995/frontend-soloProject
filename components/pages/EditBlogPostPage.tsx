...
interface EditBlogPostPageProps {
  postToEdit: BlogPostData; // 要編輯的文章數據
  navigateTo: (page: Page, data?: any) => void; // 導航函數
  onSave: (postData: BlogPostData) => void; // 儲存文章的回調
...
const EditBlogPostPage: React.FC<EditBlogPostPageProps> = ({
  ...
  // 當 `postToEdit` prop 改變時，用其數據填充表單
  useEffect(() => {
    if (postToEdit) {
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      ...
      const base64Data = await base64Promise;
      const imagePart = { inlineData: { mimeType: selectedFile?.type || 'image/jpeg', data: base64Data } };
      const prompt = `You are a professional blog writer. Analyze this image. Write a short, engaging blog post excerpt/summary about it (around 50-70 words). Respond with a single JSON object containing two keys: "contentEn" for the English content and "contentZh" for the Traditional Chinese content.`;
        ...
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <motion.div variants={fadeInUpItemVariants}>
            ...
            <div className="relative">
                ...
                required 
                className={`w-full bg-theme-tertiary border border-theme-secondary text-theme-primary rounded-md p-3 focus:${ACCENT_BORDER_COLOR} ${ACCENT_FOCUS_RING_CLASS} custom-select-text appearance-none pr-8 cursor-pointer`}
                >
