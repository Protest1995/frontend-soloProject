...
// 引入樣式常數
import { ACCENT_COLOR, ACCENT_BG_COLOR, ACCENT_BG_HOVER_COLOR } from '../../constants';
// 引入圖標組件
  ...
  const getPageNumbers = () => {
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      if (currentPage > halfPagesToShow + 2) {
        pageNumbers.push('...'); // 如果當前頁碼離第一頁很遠，顯示省略號
      }
      // 計算中間頁碼的起始和結束
      let startPage = Math.max(2, currentPage - halfPagesToShow);
      let endPage = Math.min(totalPages - 1, currentPage + halfPagesToShow);
      
      // 處理邊界情況，確保始終顯示 maxPagesToShow 個按鈕
      ...
      if (currentPage < totalPages - halfPagesToShow - 1) {
        pageNumbers.push('...'); // 如果當前頁碼離最後一頁很遠，顯示省略號
      }
      ...
      <button
        ...
        className={`p-2 rounded-md transition-colors duration-200
                    ${currentPage === 1 
                      ...
                      : `bg-theme-secondary text-theme-primary hover:bg-theme-hover ${ACCENT_BG_HOVER_COLOR} hover:text-zinc-900`
                    }`}
