...
// 引入常數和 UI 組件
import { ACCENT_BORDER_COLOR, ACCENT_FOCUS_RING_CLASS, ACCENT_COLOR } from '../../constants';
...
const CommentItem: React.FC<CommentItemProps> = ({
  ...
  // 狀態：是否已摺疊子留言 (深度大於等於2時預設摺疊)
  const [isCollapsed, setIsCollapsed] = useState(depth >= 2 && comment.children.length > 0);
  // 狀態：是否顯示刪除確認模態框
  ...
  const handleReplySubmit = (text: string) => {
    onAddComment(postId, text, comment.id); // 將回覆與父留言關聯
