...
const CommentSection: React.FC<CommentSectionProps> = ({
  ...
  const handleSubmitComment = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 調用父組件的回調函數來新增頂層留言 (parentId 為 null)
    onAddComment(postId, newCommentText, null);
    setNewCommentText(''); // 清空輸入框
    setIsSubmitting(false);
  };

  // 使用 useMemo 來緩存留言樹，只有在 comments 數組改變時才重新計算
  ...
  return (
    <div className="mt-12 pt-8 border-t border-theme-primary">
      ...
      {/* 如果用戶已登入，則顯示發表留言的表單 */}
      {isAuthenticated ? (
        <form onSubmit={handleSubmitComment} className="mb-8 p-4 bg-theme-secondary rounded-lg shadow-lg">
        ...
        // 如果用戶未登入，顯示提示信息
        <p className="mb-8 text-theme-secondary text-sm">
